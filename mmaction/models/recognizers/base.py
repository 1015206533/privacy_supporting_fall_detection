...
from mmaction.utils.xbm import XBM, DiverseXBM
from mmaction.utils.triplet_xbm import TripletLossXBM
...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.domain_head = builder.build_head(domain_head) if domain_head else None
        
        self.adaptation_pool = nn.AdaptiveAvgPool3d((1, 1, 1))
        
        assert backbone['type'] in ['IDMX3D', 'IDMResNet3d', 'IDMC3D']
        if backbone['type'] == "IDMX3D":
            self.adaptation_fc = nn.Linear(432,2048)  #X3D
            self.adaptation_fc2 = nn.Linear(2048,256) #X3D
            self.adaptation_fc3 = nn.Linear(256, 5)   #X3D
        elif backbone['type'] == "IDMResNet3d":
            self.adaptation_fc = nn.Linear(2048,1024)   #I3D
            self.adaptation_fc2 = nn.Linear(1024,256)   #I3D
            self.adaptation_fc3 = nn.Linear(256, 5)     #I3D
        else:
            self.adaptation_fc = nn.Linear(512,2048)    #C3D
            self.adaptation_fc2 = nn.Linear(2048,256)   #C3D
            self.adaptation_fc3 = nn.Linear(256, 5)     #C3D
        
        self.init_rgb_cls_loss = None 
        self.init_domain_loss = None 
        self.init_pseudo_loss = None 
        self.init_idm_feat_loss = None 
        self.init_xbm_triplet_loss = None
        self.adaptation_flag = True
        ...
        if backbone['type'] == "IDMX3D":
            self.xbm = XBM(memory_size=100, feature_size=442368)    #X3D
        elif backbone['type'] == "IDMResNet3d":
            self.xbm = XBM(memory_size=100, feature_size=262144)  #I3D
        else:
            self.xbm = XBM(memory_size=100, feature_size=41472)   #C3D
        self.diverse_xbm = DiverseXBM(memory_size=100, feature_size=2)
        self.criterion_tri_xbm = TripletLossXBM(margin=0.3)

        #logger = get_root_logger()
        #logger.info(f'test_cfg, {self.test_cfg}, {test_cfg}')
    ...
    @property
    def with_domain_head(self):
        """bool: whether the recognizer has a domain_head"""
    ...
    def init_weights(self):
        ...
        if self.with_domain_head:
            self.domain_head.init_weights()
        if self.with_neck:
            self.neck.init_weights()
        if self.adaptation_fc:
            normal_init(self.adaptation_fc, std=0.01)
        if self.adaptation_fc2:
            normal_init(self.adaptation_fc2, std=0.01)
        if self.adaptation_fc3:
            normal_init(self.adaptation_fc3, std=0.01)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if 'domain_loss_lambda' in kwargs:
            aux_info['domain_loss_lambda'] = kwargs['domain_loss_lambda']
        if 'pseudo_label_target' in kwargs:
            aux_info['pseudo_label_target'] = kwargs['pseudo_label_target']
        if 'bridge_label_target' in kwargs:
            aux_info['bridge_label_target'] = kwargs['bridge_label_target']
        if 'loss_type_list' in kwargs:
            aux_info['loss_type_list'] = kwargs['loss_type_list']
        if 'is_loss_adaptative' in kwargs:
            aux_info['is_loss_adaptative'] = kwargs['is_loss_adaptative']
