    ...
    def __call__(self, results):
        ...
        if not self.lazy:
            if 'rgb_imgs' in results:
                results['rgb_imgs'] = self._resize_imgs(results['rgb_imgs'], new_w,
                                                    new_h)
            if 'depth_imgs' in results:
                results['depth_imgs'] = self._resize_imgs(results['depth_imgs'], new_w,
                                                    new_h)
            if 'keypoint' in results:
                results['keypoint'] = self._resize_kps(results['keypoint'],
                                                       self.scale_factor)
...
class UdaNormalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            n = len(results['rgb_imgs'])
            h, w, c = results['rgb_imgs'][0].shape
            imgs = np.empty((n, h, w, c), dtype=np.float32)
            for i, img in enumerate(results['rgb_imgs']):
                imgs[i] = img

            for img in imgs:
                mmcv.imnormalize_(img, self.mean, self.std, self.to_bgr)
            
            depth_imgs = np.empty((n, h, w, c), dtype=np.float32)
            for i, img in enumerate(results['depth_imgs']):
                depth_imgs[i] = img

            for img in depth_imgs:
                mmcv.imnormalize_(img, self.mean, self.std, self.to_bgr)

            results['rgb_imgs'] = imgs
