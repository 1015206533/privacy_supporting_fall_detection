...
class FormatShape:
    ...
    def __call__(self, results):
        ...
        imgs = results['imgs']
        cls_label = results['label']
        domain_label = results['domain_label']
        # [M x H x W x C]
        ...
        if self.input_format == 'NCTHW':
            num_clips = results['num_clips']    #10
            clip_len = results['clip_len']  #16
            ...
            # M' x C x L x H x W
            # M' = N_crops x N_clips
            cls_label = [cls_label for _ in range(num_clips)]
            domain_label = [domain_label for _ in range(num_clips)]
...
class UdaFormatShape:
    ...
    def __call__(self, results):
        ...
        if not isinstance(results['depth_imgs'], np.ndarray):
            results['depth_imgs'] = np.array(results['depth_imgs'])
        rgb_imgs = results['rgb_imgs']
        depth_imgs = results['depth_imgs']

        rgb_label = results['rgb_label']
        rgb_domain = results['rgb_domain']
        depth_label = results['depth_label']
        depth_domain = results['depth_domain']
        # [M x H x W x C]
        # M = 1 * N_crops * N_clips * L
        if self.collapse:
            assert results['num_clips'] == 1
        ...
        if self.input_format == 'NCTHW':
            ...
            rgb_imgs = rgb_imgs.reshape((-1, num_clips, clip_len) + rgb_imgs.shape[1:])
            # N_crops x N_clips x L x H x W x C
            rgb_imgs = np.transpose(rgb_imgs, (0, 1, 5, 2, 3, 4))
            # N_crops x N_clips x C x L x H x W 这里 N_crops = 1
            rgb_imgs = rgb_imgs.reshape((-1, ) + rgb_imgs.shape[2:])
            # M' x C x L x H x W
            # M' = N_crops x N_clips
            rgb_label = [rgb_label for _ in range(num_clips)]
            rgb_domain = [rgb_domain for _ in range(num_clips)]

            depth_imgs = depth_imgs.reshape((-1, num_clips, clip_len) + depth_imgs.shape[1:])
            # N_crops x N_clips x L x H x W x C
            depth_imgs = np.transpose(depth_imgs, (0, 1, 5, 2, 3, 4))
            # N_crops x N_clips x C x L x H x W 这里 N_crops = 1
            depth_imgs = depth_imgs.reshape((-1, ) + depth_imgs.shape[2:])
            # M' x C x L x H x W
            # M' = N_crops x N_clips
            depth_label = [depth_label for _ in range(num_clips)]
            depth_domain = [depth_domain for _ in range(num_clips)]

        if self.collapse:
            assert rgb_imgs.shape[0] == 1
            rgb_imgs = rgb_imgs.squeeze(0)
            assert depth_imgs.shape[0] == 1
            depth_imgs = depth_imgs.squeeze(0)

        results['imgs'] = np.concatenate((rgb_imgs, depth_imgs), axis=0)
        results['input_shape'] = rgb_imgs.shape
        results['label'] = rgb_label + depth_label
        results['domain_label'] = rgb_domain + depth_domain
        return results
