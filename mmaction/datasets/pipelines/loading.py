...
class MySampleFrames:
    ...
    def __call__(self, results):
        ...
        frame_inds = np.arange(total_frames) + start_index
...
class SampleFrames:
    ...
    def __call__(self, results):
        ...
        #print('frame_inds', ','.join(map(str,list(frame_inds))))
        results['frame_inds'] = frame_inds.astype(np.int32)
        results['clip_len'] = self.clip_len #16
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = self.num_clips   #10
        return results
...
class UdaSampleFrames:
    ...
    def __call__(self, results):
        ...
        rgb_clip_offsets = self._sample_clips(rgb_total_frames)
        ...
        depth_total_frames = results['depth_total_frames']

        depth_clip_offsets = self._sample_clips(depth_total_frames)
        depth_frame_inds = depth_clip_offsets[:, None] + np.arange(
            self.clip_len)[None, :] * self.frame_interval
        depth_frame_inds = np.concatenate(depth_frame_inds)

        if self.temporal_jitter:
            perframe_offsets = np.random.randint(
                self.frame_interval, size=len(depth_frame_inds))
            depth_frame_inds += perframe_offsets

        depth_frame_inds = depth_frame_inds.reshape((-1, self.clip_len))
        if self.out_of_bound_opt == 'loop':
            depth_frame_inds = np.mod(depth_frame_inds, depth_total_frames)
        elif self.out_of_bound_opt == 'repeat_last':
            safe_inds = depth_frame_inds < depth_total_frames
            unsafe_inds = 1 - safe_inds
            last_ind = np.max(safe_inds * depth_frame_inds, axis=1)
            new_inds = (safe_inds * depth_frame_inds + (unsafe_inds.T * last_ind).T)
            depth_frame_inds = new_inds
        else:
            raise ValueError('Illegal out_of_bound option.')
        ...
        depth_frame_inds = np.concatenate(depth_frame_inds) + start_index
        results['depth_frame_inds'] = depth_frame_inds.astype(np.int32)
        results['clip_len'] = self.clip_len #16
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = self.num_clips   #10
        return results
...
class SampleProposalFrames(SampleFrames):
    ...
    def _get_test_clips(self, num_frames):
        ...
        ori_clip_len = self.clip_len * self.frame_interval
        return np.arange(
            0, num_frames - ori_clip_len, self.test_interval, dtype=np.int32)
    ...
    def __call__(self, results):
        ...
        results['frame_inds'] = np.array(frame_inds).astype(np.int32)
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = (
...
class UdaRawFrameDecode:
    ...
    def __call__(self, results):
        ...
        depth_directory = results['depth_frame_dir']
        filename_tmpl = results['filename_tmpl']
        ...
        if results['depth_frame_inds'].ndim != 1:
            results['depth_frame_inds'] = np.squeeze(results['depth_frame_inds'])
        ...
        for i, frame_idx in enumerate(results['depth_frame_inds']):
            frame_idx += offset
            filepath = osp.join(depth_directory, filename_tmpl.format(frame_idx))
            img_bytes = self.file_client.get(filepath)
            # Get frame with channel order RGB directly.
            cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
            depth_imgs.append(cur_frame)

        results['rgb_imgs'] = rgb_imgs
        results['depth_imgs'] = depth_imgs
        results['original_shape'] = rgb_imgs[0].shape[:2]
        results['img_shape'] = rgb_imgs[0].shape[:2]
